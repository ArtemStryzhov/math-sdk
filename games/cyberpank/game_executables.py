...
Правило: під час FREE SPINS, якщо на спіні випав MULTIPLIER і спін виграшний,
...
class GameExecutables(GameCalculations):
    ...
    def _sum_new_multiplier_values(self, mult_info, board_mult):
        ...
        if isinstance(mult_info, (list, tuple)):
            for item in mult_info:
                # Очікуваний формат: dict із ключем 'value'
                try:
    ...
    def set_end_tumble_event(self):
        ...
        if self.gametype == self.config.freegame_type:
            ...
            # 3) Базовий тумбл-він (на момент виклику — вже містить поточний total, бо
            #    get_scatterpays_update_wins() рахує з global_multiplier)
            ...
            if self.win_manager.spin_win > 0 and sum_new > 0:
                # ⚠️ ключова правка: передаємо у подію "базу" вже з урахуванням тоталу цього спіну,
                # щоб інваріант події залишився: updatedWin == base_win * board_mult
                base_win_for_event = base_tumble_win * float(factor_total_adjust)

                try:
                    send_mult_info_event(
                        self,
                        board_mult,                 # мультиплікатор борда
                        mult_info,                  # деталі по клітинках
